...
문장은 새로 쓴다 (calculator.net 번역/복붙 금지).
...
폰트가 다른 박스와 어긋남). 숫자를 대입해 보는 "예시 계산"만 `.formula-example`(작은
모노스페이스) 사용, 그 외 진짜 공식은 전부 `.formula-box`로 승격.
...
- **`criteria-table`**: 항목이 한 축 위에서 순서를 이룸 (예: BMI 저체중→정상→과체중→비만). 마지막 열 기본값은 숫자용(모노스페이스+우측정렬)이니, 문장이 들어가면 `class="criteria-table text-table"`로 감싸 일반 한글 폰트+좌측정렬로 바꿀 것.
...
기존 클래스로 표현 안 되는 새 스타일이 필요하면 추가 전에 사용자 확인.
