...
모노스페이스로 따로 두지 않는다 — 그냥 일반 `<p>`로 쓴다. 진짜 공식만 `.formula-box`로 승격.
...
둘 다 있는 페이지에만** 넣는다. 결과가 몇 줄짜리 `.stat-table` 요약뿐인 페이지(BMI·BMR 등)는
뽑을 데이터 자체가 없으니 CSV 버튼을 넣지 않는다. 새 계산기가 이 기준(다중 행 표 + 그래프)에
해당하면 그때 추가한다.
...
- `#copyResultBtn`은 `.page-title h1` 제목 + `.screen .label`/`#miniScreen`/
  ...
  동작한다.
- 대출·할부·1RM처럼 결과가 `.receipt-strip .method-card`(방식별 비교 카드,
  `<h3>{방식명}</h3>` + 선택적 `.stamp` 뱃지 + `.method-row><span>{라벨}</span>
  <span class="val">{값}</span>`)로 여러 개 나오는 페이지는, 대표값 한 줄 다음에 카드마다
  방식명(뱃지 있으면 괄호로 병기) + 라벨:값 줄을 전부 붙인다 — 이것도 마크업 클래스만
  맞으면 자동으로 잡히니 새 비교형 계산기를 만들 때 `.method-card`/`.method-row`/`.val`
  구조만 그대로 쓰면 별도 JS 없이 결과 복사에 전체 비교표가 포함된다.
