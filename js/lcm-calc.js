...
function recalc(){
  const raw = document.getElementById('lc-nums').value;
  ...
  miniScreenSub.textContent = `${nums.join(', ')}의 최소공배수`;
  meta.textContent = nums.join(', ');

  statBody.innerHTML = `
    <tr><th>입력한 수</th><td>${nums.join(', ')}</td></tr>
    <tr class="stat-highlight"><th>최소공배수 (LCM)</th><td>${fmt(result)}</td></tr>
  `;
...
const URL_DEFAULTS = {
  nums: document.getElementById('lc-nums').defaultValue
};

const urlParams = UrlState.read();
if (urlParams.nums) document.getElementById('lc-nums').value = urlParams.nums;

document.getElementById('lc-nums').addEventListener('input', recalc);
